...
const CACHE_REFRESH_INTERVAL_MINUTES = Number(
  Deno.env.get("CACHE_REFRESH_INTERVAL_MINUTES") ?? "30",
);
...
export const startCacheRefreshScheduler = (kv: Deno.Kv): void => {
  ...
  console.log(
    `Starting cache refresh scheduler (every ${CACHE_REFRESH_INTERVAL_MINUTES} minutes)`,
  );
