...
export const handler = define.handlers({
  GET: async (ctx) => {
    const images = await getImagesByAlbum(ctx.state.kv, "when-the-light-hits") ?? [];
    return { data: images };
  },
});

export default define.page<typeof handler>(function WhenTheLightHits({ data }) {
  const images = data;

  return (
    <div class="px-4 py-8 mx-auto md:min-h-screen bg-gray-50">
      <Head>
        <title>When the Light Hits - Dana Gill Photography</title>
      </Head>
      <div class="max-w-7xl mx-auto">
        <PageHeader
          title="When the Light Hits"
          subtitle="Coincidences in life and light. 2022 - Current."
        />
